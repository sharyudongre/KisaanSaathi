...
class ChatInterface(Frame):
    ...
    def __init__(self, master=None):
        ...
        help_option.add_command(label="About KisaanSaathi", command=self.msg)
        ...
        # t2.start()
        x = "I am KisaanSaathi,I can assist you with weed management." + "\n"
        y = "मैं किसानसाथी हूं, मैं खरपतवार प्रबंधन में आपकी सहायता कर सकता हूं।" + "\n"
        self.text_box.configure(state=NORMAL)
        self.text_box.insert(END, x)
        self.text_box.configure(state=DISABLED)
        self.text_box.see(END)

        t2 = threading.Thread(target=self.playResponce, args=(x,))
        t2.start()


        self.text_box.configure(state=NORMAL)
        self.text_box.insert(END, y)
        self.text_box.configure(state=DISABLED)
        self.text_box.see(END)
    ...
    def msg(self):
        tkinter.messagebox.showinfo("KisaanSaathi",
                                    'KisaanSaathi is a chatbot for answering queries related to weed management.\nIt is based on retrival-based NLP using pythons NLTK tool-kit module.\nGUI is based on Tkinter.')

    def about(self):
        tkinter.messagebox.showinfo("KisaanSaathi Developers",
                                    "1.Sharyu Dongre\n2.Shubhagyata Swaraj Jayswal\n3.Rashmikiran Pandit\n4.Payal")
